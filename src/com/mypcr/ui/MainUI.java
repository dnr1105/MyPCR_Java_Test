...
/**
 * 처음에 나타나는 UI에 대한 클래스이다. JFrame을 상속받고 {@link Main} 클래스에서 호출하여 실행된다.
 * 
 * @author YJ
 * 
 */
public class MainUI extends JFrame implements Handler, DeviceChange, KeyListener {
	...
	/**
	 * {@link HIDManager} 의 OpenById 함수를 통해 장치와 연결되면, 연결된 장치를 저장한다. 이 객체는 장치와 Write, Read 통신이 가능하고 통신에서 Blocking, Non-Blocking 처리를 제어한다.
	 * 
	...
	/**
	 * Frame에 구성요소들을 담고 있는 Swing Panel 이다. 이 Panel 에 add 함수를 이용하여 Swing 컴포넌트들을 더하면 Frame에 표시된다.
	 */
	...
	/**
	 * Protocol List에 저장된 Action들을 저장하는 배열이다. Action은 Label, Temp, Time, Remaining Time 값을 가지고 있다.
	 * 
	 * @see Action
	 */
	private Action[] m_ActionList = null;
	/**
	 * PCR 을 제어하기 위한 기능들을 가지고 있는 객체이다. 전체 클래스에서 가장 중요한 역할을 한다.
	 * 
	 * @see PCR_Task
	 */
	private PCR_Task m_PCRTask = null;
	/**
	 * PCR과 연결 상태를 나타내는 플래그. 연결되면 true, 연결이 해제되면 false.
	 */
	private boolean IsConnected = false;
	/**
	 * Protocol 파일을 읽었는지 확인하는 플래그. 읽었으면 true, 읽은 적이 없거나 오류가 생기면 false.
	 */
	private boolean IsProtocolRead = false;
	/**
	 * 정지가 된적이 있는지 확인하는 플래그. 정지된 적이 없으면 true, 있으면 false. 정지된 적이 없으면 처음 실행한 것이 되기 때문에, 
	 * 처음 실행한 것이 되기 때문에 true가 되는 경우는 프로그램이 처음 켜졌을 경우만 해당한다.
	...
	/**
	 * MainUI 기본 생성자이다. private 설정으로 생성자를 호출할 수 없도록 되어 있고, getInstance() 함수를 통하여 인스턴스를 생성할 수 있다.(Singleton 기법).
	...
	/**
	 * UI를 초기화 하거나, 객체들을 초기화 하는 역할을 한다. MainUI() 생성자에서만 호출할 수 있다. 단 한번만 호출된다.
	 */
	private void init() {
		// 프레임의 크기 지정
	...
	/**
	 * main 함수에서 UI를 띄우기 위해 맨 처음 호출하는 함수. Singleton 기법의 getInstance와 같은 함수이다. 어법상 말이 Run이 더 어울리기 때문에 Run으로 정하였다.
	 * 
	...
	/**
	 * HIDManager 를 통하여 연결한 HIDDevice 객체를 리턴한다. 연결되어 있을 떄(Not null), 다른 클래스에서 이 함수를 통하여 PCR 장치와 통신을 할 수 있다.
	 * 
	 * @see HIDManager
	 */
	...
	/**
	 * 웹 서버로부터 펌웨어 버전을 체크하고, 펌웨어 업데이트를 할 수 있도록 하는 함수. PCR과 연결이 된 상태에서 맨 처음에 한번만 체크합니다. 펌웨어 버전을 서버에서 받아온 후, 
	 * 그 버전과 PCR 버전과 같은지 확인 후 다르면 웹서버로부터 새로운 펌웨어를 받아서 PCR에 업로드 시킵니다. 업로드가 완료되면
	 * PCR을 Reset 시킵니다. 웹 서버가 꺼져있으면 연결할 수 없다는 메시지가 나옵니다. 이 기능이 제대로 동작하려면 다른 BootLoader 프로그램을 종료하십시오.
	 * 
	 * @see BootLoader
	 */
						...
						while (true) 
						{
							...
							while (!isStartEndFlag) {
								try {
									...
									Thread.sleep(temp);
									System.out.println(temp/1000+"초");
								} catch (InterruptedException e) {}
							}
